...
int main() {
  ...
  for (int i = 0; i < cmd.size(); i++) {
    if (cmd.at(i) == "|" || i == cmd.size() - 1) {
      if (cmd.at(i) == "|") {
        this_vbar =
            std::find(vbar_pos.begin(), vbar_pos.end(), i) - vbar_pos.begin();
        last_vbar =
            this_vbar - 1; /*this_vbar, last_vbar are indexes in vbar_pos*/
      }
      if (pipe(p1) < 0) std::cout << "create pipe1 error" << std::endl;
      if (pipe(p2) < 0) std::cout << "create pipe2 error" << std::endl;
      signal(SIGCHLD, SIG_IGN);
      if (fork() == 0) { /*child process: call execvp if cmd.at(i) is "|" or is
                            the last word*/
        if (i == cmd.size() - 1) {
          close(p1[1]);
      ...
      } else { /*parent process: pass old data to the new child and store new
                  data*/
        close(p1[0]);
